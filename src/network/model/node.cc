...
Node::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Node")
    ...
    .AddAttribute ("NodeType",
                   "The type of the node, either NPU or SWITCH",
                   EnumValue (SWITCH), // 默认值是 SWITCH
                   MakeEnumAccessor (&Node::GetNodeType, &Node::SetNodeType),
                   MakeEnumChecker (NPU, "NPU",
                                    SWITCH, "SWITCH",
                                    INNER_L1, "INNER_L1",
                                    INNER_L2, "INNER_L2",
                                    INNER_L3, "INNER_L3",
    ...
    .AddAttribute("AreaId",
                   "The area id of this node: a unique integer used for parallel simulations.",
                   TypeId::ATTR_GET | TypeId::ATTR_SET,
                   UintegerValue (0),
                   MakeUintegerAccessor (&Node::m_areaId),
                   MakeUintegerChecker<uint32_t> ()
    )
  ;
  return tid;
...
void 
Node::SetAreaId(uint32_t areaId){
  m_areaId = areaId;
} 

uint32_t 
Node::GetAreaId() const{
  return m_areaId;
}
