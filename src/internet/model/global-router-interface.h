 ...
 * @param linkData The link data field for the record.
 * @param metric The metric field for the record.
 * @param areaId The area ID of the record.
 * @param nodeType The node type of the record.
 * @see LinkType
 * @see SetLinkId
 * @see SetLinkData
 */
 ...
 */
  ...
  Node::NodeType GetNodeType (void) const;

  void SetNodeType (Node::NodeType nodeType);  
 ...
 * LSU报文头部LSA头部的建模，会记录链路状态类型链路状态等信息，以及通告路由器等
 */
