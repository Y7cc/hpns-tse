...
{ 
  ...
  NS_LOG_INFO("BuildGlobalRoutingDatabase begin");
  Logger::getLogger("BuildGlobalRoutingDatabase")->info("BuildGlobalRoutingDatabase begin");
  NS_LOG_FUNCTION (this);
...
//
    ...
    Time buildDatabaseDuration = endTime - startTime ;
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration = end - start;
    // std::cout << "------BuildGlobalRoutingDatabase time: " << duration.count() << " seconds------" << std::endl;
    Logger::getLogger("BuildGlobalRoutingDatabase")->trace("BuildGlobalRoutingDatabase time: {} seconds", duration.count());
    Logger::getLogger("BuildGlobalRoutingDatabase")->info("BuildGlobalRoutingDatabase end");
    // std::cout << "------BuildGlobalRoutingDatabase time： " << buildDatabaseDuration.GetSeconds() << " seconds------" << std::endl;
}
...
//
  ...
  Logger::getLogger("InitializeRoutes")->info("Start SPF calculation");
...
//    
      if (rtr && rtr->GetNumLSAs () )
        {
          // NS_LOG_INFO ("Node：" << count++ << "SPF calculation begin");
          SPFCalculate (rtr->GetRouterId ());
          // NS_LOG_INFO ("Node：" << count++ << "SPF calculation finished");
        }
    }
  NS_LOG_INFO ("Finished SPF calculation");
  Logger::getLogger("InitializeRoutes")->info("Finished SPF calculation");
  Time endTime = Simulator::Now();
  Time InitializeRoutesDuration = endTime - startTime;
  auto end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> duration = end - start;
  // std::cout << "------InitializeRoutes time: " << duration.count() << " seconds------" << std::endl;
  Logger::getLogger("InitializeRoutes")->trace("InitializeRoutes time: {} seconds", duration.count());
  // std::cout << "------InitializeRoutes time： " << InitializeRoutesDuration.GetSeconds() << " seconds------" << std::endl;
}
...
//
  if (v->GetVertexType () == SPFVertex::VertexRouter)
    {
      numRecordsInVertex = v->GetLSA ()->GetNLinkRecords (); //邻接链路的数量
    }
  if (v->GetVertexType () == SPFVertex::VertexNetwork)
    {
      numRecordsInVertex = v->GetLSA ()->GetNAttachedRouters (); //邻接路由器的数量
    }
...
// shortest path calculation.  如果是末梢网络先跳过 在spf第二阶段考虑
//
...
// database. 如果是中转路由器或者网络 则寻找W的LSA
//
...
//
              w_lsa = m_lsdb->GetLSA (l->GetLinkId ());  //(l->GetLinkId ()  对端Router的ID
              NS_ASSERT (w_lsa);   //w_lsa  对端router的lsa
...
// to be run
// check node是否为stub node ,即只有一个接口与其他节点相连接，如果是则可以添加静态路由跳过spf计算
bool
GlobalRouteManagerImpl::CheckForStubNode (Ipv4Address root)
{
  NS_LOG_FUNCTION (this << root);
  GlobalRoutingLSA *rlsa = m_lsdb->GetLSA (root);
  Ipv4Address myRouterId = rlsa->GetLinkStateId (); //routeId
  int transits = 0;
  ...
  if (transits == 1)
    {
      if (transitLink->GetLinkType () == GlobalRoutingLinkRecord::TransitNetwork)
      ...
      else if (transitLink->GetLinkType () == GlobalRoutingLinkRecord::PointToPoint)
        {
          ...
          GlobalRoutingLSA *w_lsa = m_lsdb->GetLSA (transitLink->GetLinkId ());  //获取链路对面路由器的LSA
              ...
              if (lr->GetLinkId () == myRouterId)
                {
                  ...
                  Ptr<GlobalRouter> router = rlsa->GetNode ()->GetObject<GlobalRouter> (); //root节点的GlobalRouter对象
                  NS_ASSERT (router);
                  Ptr<Ipv4GlobalRouting> gr = router->GetRoutingProtocol ();
                  NS_ASSERT (gr);
...
bool
GlobalRouteManagerImpl::CheckForNpuNode (Ipv4Address root)
{
  NS_LOG_FUNCTION (this << root);
  GlobalRoutingLSA *rlsa = m_lsdb->GetLSA (root);
  Ipv4Address myRouterId = rlsa->GetLinkStateId ();
  //获取根节点对应的node对象指针
  Ptr<Node> rnode = rlsa->GetNode ();
  EnumValue nodeType;
  rnode->GetAttribute ("NodeType", nodeType);
  if (nodeType.Get () == Node::NPU)
  {    
      std::cout << root <<" is NPU" << std::endl;
      // NPU node, check if it is a stub node
      if(CheckForStubNode (root)){
        return true;
      }else{
        //not stub node, build routing table;
        uint32_t nLinks = rlsa->GetNLinkRecords();
        for(uint32_t i=0; i<nLinks; i++){
          GlobalRoutingLinkRecord *lr = rlsa->GetLinkRecord(i);
          if(lr->GetLinkType() == GlobalRoutingLinkRecord::PointToPoint){
              GlobalRoutingLSA *w_lsa = m_lsdb->GetLSA (lr->GetLinkId ());  //获得链路对端router的lsa
              uint32_t nLinkRecords = w_lsa->GetNLinkRecords();

              //遍历对端router 的lsa record，Find the link record that corresponds to our routerId
              for(uint32_t j=0; j<nLinkRecords; j++){
                  GlobalRoutingLinkRecord *lr2 = w_lsa->GetLinkRecord(j);

                  //只关心点对点类型
                  if(lr2->GetLinkType() != GlobalRoutingLinkRecord::PointToPoint){
                      continue;
                  }

                  if(lr2->GetLinkId() == myRouterId){
                      Ptr<GlobalRouter> router = rlsa->GetNode()->GetObject<GlobalRouter>(); //root节点的GlobalRouter对象
                      NS_ASSERT (router);
                      Ptr<Ipv4GlobalRouting> gr = router->GetRoutingProtocol ();  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                      NS_ASSERT (gr);
                      gr->AddNetworkRouteTo (lr2->GetLinkData(), Ipv4Mask("255.255.255.255") ,lr2->GetLinkData(), 
                                            FindOutgoingInterfaceId(lr->GetLinkData ()));
                      NS_LOG_LOGIC ("Inserting default route for node " << myRouterId << " to next hop " << 
                                  lr2->GetLinkData() << " via interface " << 
                                  FindOutgoingInterfaceId (lr->GetLinkData ()));
                      break;
                  } 
              }
...
//
  uint32_t systemId = MpiInterface::GetSystemId();
  m_spfroot= v;   //root node
  v->SetDistanceFromRoot (0);
  v->GetLSA ()->SetStatus (GlobalRoutingLSA::LSA_SPF_IN_SPFTREE);
  NS_LOG_LOGIC ("Starting SPFCalculate for node " << root);
  Ptr<Node> rootNode = m_lsdb->GetLSA (root)->GetNode();
  uint32_t rootAreaId = rootNode->GetAreaId();
  // std::cout << "root area id : " << rootAreaId << std::endl;
  Node::NodeType rootNodeType = rootNode->GetNodeType();
  // std::cout << "root node type: " << rootNodeType << std::endl;
  Logger::getLogger("SPF")->info("Process {} Starting SPFCalculate for node {}",systemId, rootNode->GetId());

//先为NPU节点添加默认路由
  if(rootNodeType == Node::NPU){
    // std::cout << "root node" << rootNode->GetId() << " is NPU add default route" << std::endl;
    Logger::getLogger("SPF")->info("root node {} is NPU add default route", rootNode->GetId());
    //遍历周围节点 如果是OUTER L1节点，则添加为默认路由
    GlobalRoutingLSA *rlsa = m_lsdb->GetLSA (root);
    ...
    for(uint32_t i=0; i<rlsa->GetNLinkRecords(); i++){
      ...
      GlobalRoutingLinkRecord *lr = rlsa->GetLinkRecord(i);
      if(lr->GetLinkType() == GlobalRoutingLinkRecord::PointToPoint){
        Ipv4Address remoteRouterId = lr->GetLinkId();
        GlobalRoutingLSA *w_lsa = m_lsdb->GetLSA (remoteRouterId);
        // std::cout << "remote router type : " << w_lsa->GetNodeType() << std::endl;
        // std::cout << (w_lsa->GetNodeType() == Node::OUTTER_L1) << std::endl;
        if(w_lsa->GetNodeType() == Node::OUTTER_L1){
          uint32_t nLinkRecords = w_lsa->GetNLinkRecords();

          for(uint32_t j = 0; j<nLinkRecords; j++){
            GlobalRoutingLinkRecord *remoteLr = w_lsa->GetLinkRecord(j);

            if(remoteLr->GetLinkType() != GlobalRoutingLinkRecord::PointToPoint){
              continue;
            }

            if(remoteLr->GetLinkId() == root){
              Ptr<GlobalRouter> rootRouter = rootNode->GetObject<GlobalRouter>();
              NS_ASSERT (rootRouter);
              Ptr<Ipv4GlobalRouting> gr = rootRouter->GetRoutingProtocol ();
              NS_ASSERT (gr);
              gr->AddNetworkRouteTo (Ipv4Address("0.0.0.0"), Ipv4Mask("0.0.0.0") , remoteLr->GetLinkData(), 
                                    FindOutgoingInterfaceId(lr->GetLinkData ()));
              NS_LOG_LOGIC ("Inserting default route for node " << root << " to next hop " << 
                            lr->GetLinkData() << " via interface " << 
                            FindOutgoingInterfaceId (lr->GetLinkData ()));
              break;
            }
...
//如果某一个Node只有一个接口链接一个路由器，则不需要为其计算spf.
  if (NodeList::GetNNodes () > 0 && CheckForStubNode (root))
    ...
    //   if (NodeList::GetNNodes () > 0 && CheckForNpuNode (root))
    // {
    //   NS_LOG_LOGIC ("SPFCalculate truncated for npu node " << root);
    //   delete m_spfroot;
    //   return;
    // } 
...
// //
//       Ptr<Node> vnode =  v->GetLSA()->GetNode();
//       EnumValue vnodeType;
//       vnode->GetAttribute ("NodeType", vnodeType);

      uint32_t desAreaId = v->GetLSA()->GetAreaId();
      Node::NodeType desNodeType = v->GetLSA()->GetNodeType();

      if(rootNodeType == Node::NPU){
        // std::cout << "NPU node SPFCalculate" << std::endl;
        //如果目前计算的根节点是npu节点 则按照NPU节点的路由生成规则--- 只存储到达同区域的npu节点 以及默认的OUTERL1路由器的路由
        if((v->GetVertexType () == SPFVertex::VertexRouter) && (desNodeType == Node::NPU) && (desAreaId == rootAreaId)){
        // std::cout << "NPU node SPFCalculate" << std::endl;
          SPFProcessStubs (v);
        }else if (v->GetVertexType () == SPFVertex::VertexNetwork ){
        ...
        //   NS_LOG_WARN ("illegal SPFVertex type SKIP");
        // }

      }else if((rootNodeType != Node::NPU) && ((rootNodeType == Node::INNER_L1) || (rootNodeType == Node::INNER_L2) || (rootNodeType == Node::INNER_L3))){
        // std::cout << "INNER node SPFCalculate" << std::endl;
        //当前根节点不是npu节点 但是Inner交换机节点则按照Inner交换机节点的路由生成规则--- 存储到达同区域的npu节点
        if(v->GetVertexType () == SPFVertex::VertexRouter && desNodeType == Node::NPU && desAreaId == rootAreaId){
          SPFProcessStubs (v);
        }else if (v->GetVertexType () == SPFVertex::VertexNetwork ){
          SPFProcessStubs (v);
        ...
        // std::cout << "OUTER node SPFCalculate" << std::endl;
        //当前节点不是npu节点 但是Outter交换机节点则按照Outter交换机节点的路由生成规则--- 存储到达所有npu节点的路由
        if((v->GetVertexType () == SPFVertex::VertexRouter) && (desNodeType == Node::NPU)){
          // SPFIntraAddRouter (v);
          SPFProcessStubs (v);
        }
        // else if (v->GetVertexType () == SPFVertex::VertexNetwork ){
