...
Ipv4ListRouting::RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif, enum Socket::SocketErrno &sockerr)
{
  ...
  NS_LOG_LOGIC ("");
//没有找到路由，则检查当前节点是否为NPU节点
  // if (oif == nullptr)
  // {
  //   std::cout << "oif nullptr!!!!!!!!! "<< std::endl;
  // }
  ...
  // std::cout << "client node id: " << node->GetId() << std::endl;
  // Ipv4RoutingProtocolList::const_iterator i = m_routingProtocols.begin ();
  // std::advance(i, 1);
  // Ptr<Ipv4GlobalRouting> globalRouting = DynamicCast<Ipv4GlobalRouting>((*i).second);
  // if (globalRouting == nullptr) {
  //   // NS_LOG_ERROR("Ipv4GlobalRouting is nullptr, cannot perform lookup.");
  //   std::cout << "Ipv4GlobalRouting is nullptr, cannot perform lookup."<< std::endl;
  //   sockerr = Socket::ERROR_NOROUTETOHOST;
  //   return 0;
  // }

  // EnumValue nodeType;
  // node->GetAttribute ("NodeType", nodeType);

  // if(nodeType.Get () == Node::NPU){
  //     //节点是NPU节点
  //     std::cout << "NPU node detected, forwarding packet to neighbors"<< std::endl;
  //     NS_LOG_LOGIC ("NPU node detected, forwarding packet to neighbors");
  //     for(uint32_t i=0; i<node->GetNDevices(); i++){
  //         //便利每一个netdevice 获取channel
  ...
  //                 Ptr<NetDevice> neighborDev = ch->GetDevice (j);
  //                 if (neighborDev != dev){
                      
  //                     // std::cout << "ifIndex:  "<< ifIndex << std::endl;
  //                     Ptr<Node> neighborNode = neighborDev->GetNode ();
  //                     EnumValue neighborNodeType;
  //                     neighborNode->GetAttribute ("NodeType", neighborNodeType);
  //                     // if(neighborNodeType.Get() != Node::ACCSW){
  //                     //   std::cout << "neighbor node is not NPU node, skipping"<< std::endl;
  //                     //   break;
  //                     // }

  //                     std::cout << "neighborNodeid: " << neighborNode->GetId() << std::endl;

  //                     Ptr<Ipv4> neighborIpv4 = neighborNode->GetObject<Ipv4>();

  //                     uint32_t ifIndex = neighborIpv4->GetInterfaceForDevice(neighborDev);  //获取了该deVice的接口号


  //                     Ptr<Ipv4Route> neighborRoute = neighborIpv4->GetRoutingProtocol()->RouteOutput(p, header, oif, sockerr);
  //                       if (neighborRoute == nullptr)
  //                       {
  //                         std::cout << "neighborRoute nullptr!!!!!!!!! "<< std::endl;
  //                       }

  //                     if (neighborRoute){
  //                         NS_LOG_LOGIC ("Found route in neighbor: " << neighborRoute << ", Return route to neighbor");
  //                         std::cout << "Found route in neighbor: " << neighborRoute << ", Return route to neighbor"<< std::endl;
  //                         std::cout << "neighborRoute destination： " << neighborRoute->GetDestination() << "source: " << neighborRoute->GetSource() << "gateway: "<< neighborRoute->GetGateway() << std::endl;
  //                         Ipv4InterfaceAddress neighborAddress = neighborIpv4->GetAddress(ifIndex , 0); // 0为接口索引
  //                         Ipv4Address neighborip = neighborAddress.GetLocal(); // 获取IP
  //                         NS_LOG_INFO("neighbor IP Address: " << neighborip);
  //                         // std::cout << "neighbor IP Address: " << neighborip<< std::endl;
  //                         Ptr<Ipv4Route> routeToNeighbor = Create<Ipv4Route> ();
  //                         routeToNeighbor = globalRouting->LookupGlobal(neighborip);

  //                         if(routeToNeighbor == nullptr){
  //                           std::cout << "routeToNeighbor nullptr!!!!!!!!! "<< std::endl;
  //                         }

  //                         std::cout << "routeToNeighbor destination： " << routeToNeighbor->GetDestination() << "source: " << routeToNeighbor->GetSource() << std::endl;

  //                         sockerr = Socket::ERROR_NOTERROR;
  //                         return routeToNeighbor;  // 返回当前节点到邻居节点的路由
  //                     }
  //                 }
  //             }
  //         }
...
void Ipv4ListRouting::SetNode (Ptr<Node> node){
  m_node = node;
}
