...
GlobalRoutingLinkRecord::GlobalRoutingLinkRecord (
  ...
  :
    ...
    m_nodeType (nodeType)
{
  NS_LOG_FUNCTION (this << linkType << linkId << linkData << metric << areaId << nodeType);
}
...
uint32_t
GlobalRoutingLinkRecord::GetAreaId (void) const{
  NS_LOG_FUNCTION (this);
  return m_areaId;
}

void
GlobalRoutingLinkRecord::SetAreaId (uint32_t areaId){
  NS_LOG_FUNCTION (this << areaId);
  m_areaId = areaId;
}

Node::NodeType 
GlobalRoutingLinkRecord::GetNodeType (void) const{
  NS_LOG_FUNCTION (this);
  return m_nodeType;
}

void
GlobalRoutingLinkRecord::SetNodeType (Node::NodeType nodeType){
...
GlobalRoutingLSA::GlobalRoutingLSA (GlobalRoutingLSA& lsa)
  : m_lsType (lsa.m_lsType), m_linkStateId (lsa.m_linkStateId),
    ...
    m_areaId (lsa.m_areaId),
    m_nodeType (lsa.m_nodeType)
{
...
uint32_t
GlobalRoutingLSA::GetAreaId (void) const{
  NS_LOG_FUNCTION (this);
  return m_areaId;
}

void
GlobalRoutingLSA::SetAreaId (uint32_t areaId){
  NS_LOG_FUNCTION (this << areaId);
  m_areaId = areaId;
}

Node::NodeType 
GlobalRoutingLSA::GetNodeType (void) const{
  NS_LOG_FUNCTION (this);
  return m_nodeType;
}

void
GlobalRoutingLSA::SetNodeType (Node::NodeType nodeType){
  NS_LOG_FUNCTION (this << nodeType);
  m_nodeType = nodeType;
}
...
GlobalRoutingLSA::CopyLinkRecords (const GlobalRoutingLSA& lsa)
{
  ...
  for (ListOfLinkRecords_t::const_iterator i = lsa.m_linkRecords.begin ();
       ...
       i++)
    {
      ...
      pDst->SetAreaId (pSrc->GetAreaId ());
      pDst->SetNodeType (pSrc->GetNodeType ());
...
GlobalRouter::DiscoverLSAs ()
{
  ...
  pLSA->SetNode (node);
  pLSA->SetNodeType (node->GetNodeType ());
  pLSA->SetAreaId (node->GetAreaId ());
...
GlobalRouter::ProcessPointToPointLink (Ptr<NetDevice> ndLocal, GlobalRoutingLSA *pLSA)
{
  ...
  uint32_t interfaceRemote = ipv4Remote->GetNInterfaces () + 1;
    // uint32_t interfaceLocal = ipv4Remote->GetNInterfaces () + 1
  ...
  if (ipv4Remote->IsUp (interfaceRemote))
    {
      ...
      plr->SetLinkData (addrLocal);
      plr->SetMetric (metricLocal);
      plr->SetAreaId (nodeRemote->GetAreaId());
      plr->SetNodeType (nodeRemote->GetNodeType());
      pLSA->AddLinkRecord (plr);
      plr = 0;
    }

  // Regardless of state of peer, add a type 3 link (RFC 2328: 12.4.1.1)
  //修改了stubnetwork的linkrecord 以简化同一个节点生成的路由条目。
  plr = new GlobalRoutingLinkRecord;
  NS_ABORT_MSG_IF (plr == 0, "GlobalRouter::ProcessPointToPointLink(): Can't alloc link record");
  plr->SetLinkType (GlobalRoutingLinkRecord::StubNetwork);
  plr->SetLinkId (addrLocal);
  plr->SetLinkData (Ipv4Address (maskLocal.Get ()));  // Frown
  plr->SetMetric (metricLocal);
  plr->SetAreaId (nodeRemote->GetAreaId());
  plr->SetNodeType (nodeRemote->GetNodeType());
  pLSA->AddLinkRecord (plr);
